...
txt_file_path = Path(plugin_path, "data")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if page == 1:
                    ...
                    #total_pages = 3 # Ceiling division
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for tag in tags:
                ...
                tag_parents = tag.get("parents", [])

                tag_info = {"id": tag_id, "name": tag_name, "aliases": tag_aliases, "description": tag_description, "stash_ids": tag_stash_ids, "parents": tag_parents}
...
def write_existing_tags_master(existing_tag_map, filename="existing_tags_master.json"):
    ...
    try:
        ...
        for info in existing_tag_map.values():
            ...
            if not tag_id:
                continue
            entry = tags_by_id.setdefault(tag_id, {"id": tag_id, "name": info.get("name"), "description": info.get("description", ""), "aliases": set(), "stash_ids": [], "parents": []})
            # merge aliases
            ...
            # append parents objects (may contain endpoint/stash_id)
        ...
        for entry in tags_by_id.values():
           ...
           # dedupe stash_ids by tuple(endpoint, stash_id)
            ...
            for p in entry.get("parents", []):
                key2 = (p.get("id"), p.get("name"), p.get("description"))
                if key2 not in seen2:
                    seen2.add(key2)
                    deduped_parents.append({"id": p.get("id"), "name": p.get("name"), "description": p.get("description")})
            ...
            c = 0
            #for p in entry.get("parents", []):
            for p in deduped_parents:
                if p:
                  c += 1
                  if c == 1:
                    parent1 = str(p.get("name" or "")) + " | " + str(p.get("description" or ""))

                  if c == 2:
                    parent2 = str(p.get("name" or "")) + " | " + str(p.get("description" or ""))

                  if c == 3:
                    parent3 = str(p.get("name" or "")) + " | " + str(p.get("description" or ""))


                    #parent_name = str(p.get("name" or ""))
                    #parent_description = str(p.get("description" or ""))
        ...
        data_path = Path(txt_file_path,filename)
...
def write_stashdb_tags_master(stashdb_tags, filename="stashdb_tags_master.json"):
    """
    Write a deduplicated JSON file of existing tags (unique by local id) to the cwd.
    The output contains id, name, aliases, and stash_ids for each tag.
    """
    try:
        ...
        for entry in stashdb_tags:
            aliases = sorted(list(entry.get("aliases", [])))
            stashdb_id = str(entry.get("stash_id", entry.get("id") or ""))
            #entry_category = sorted(list(entry.get("category", [])))
            entry_category = entry.get("category", "")
            #entry_category = entry.get("category", "")
            #category_name = str(entry_category.get("name" or ""))
            #category_group = str(entry_category.get("group" or ""))
            #category_description = str(entry_category.get("description" or ""))


            category_name = ""
            category_group = ""
            category_description = entry_category

            #if not entry.get("category"):
              #category_description = "frog"

            #category_description = entry.get("category")
            if not entry_category is None:
              #category_group = len(entry_category)
              #category_name = len(entry_category)
              category_name = str(entry_category.get("name" or ""))
              category_group = str(entry_category.get("group" or ""))
              category_description = str(entry_category.get("description" or ""))
               #category_group = "its none"
                #for e in entry_category:
                #category_name = len(e)
                #category_group = e
                #category_description = str(e.get("description" or ""))
                #if e and e.get("name"):
                #category_name = str(e.get("name" or ""))
                #category_name = e.get("name")
                #if e and isinstance(e, dict) and e.get("name"):
                  #category_name = "fuck"


            #if len(entry_category) == 0:
              #category_group = "length"


            #if entry.get("category"):
              #category_name = "cat"
              #category_name = str(entry.get("name" or ""))
              #category_group = str(entry.get("group" or ""))
              #category_description = str(entry.get("description" or ""))


              #for e in entry.get("category", []):
                #if e and e.get("name"):
                  #category_name = str(e.get("name" or ""))
                  #category_name = e.get("name")
                  #if e and isinstance(e, dict) and e.get("name"):
              ...
              #  if e and e.get("name"):
              #    category_name = str(e.get("name" or ""))
              #    category_group = str(e.get("group" or ""))
              #    category_description = str(e.get("description" or ""))
        ...
        log.info(f"Wrote stashdb tags master JSON to {filename} ({len(output_list)} tags).")
    except Exception as e:
        log.error(f"Failed to write stashdb tags master JSON: {e}")
...
def main():
    ...
    # Dump the existing tags master JSON for offline comparison / delta calculation
    write_stashdb_tags_master(stashdb_tags, "stashdb_tags_master.json")
